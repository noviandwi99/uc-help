...
class UC:
	...
	def inputDataExcel(self):
		for sheetName in self.dataList:
			if sheetName=='standardGenData':
				self.standardGenData=(pd.read_excel(self.excelFile,sheetName).loc[[0,7,8,9],:]).to_numpy()
			elif sheetName=='costGenData':
				self.costGenData=(pd.read_excel(self.excelFile,sheetName).loc[[0,7,8,9],:]).to_numpy()
			elif sheetName=='freqRegulationGenData':
				self.freqRegulationGenData=(pd.read_excel(self.excelFile,sheetName).loc[[0,7,8,9],:]).to_numpy()
			elif sheetName=='continuityGenData':
				self.continuityGenData=(pd.read_excel(self.excelFile,sheetName).loc[[0,7,8,9],:]).to_numpy()
			elif sheetName=='loadData':
				self.loadData=(pd.read_excel(self.excelFile,sheetName).loc[[1],:]).to_numpy()
				...
				self.busLoad=(pd.read_excel(self.excelFile,sheetName).loc[[0,1,2,8,24,25,26,27,28,29,36,37,38],:]).to_numpy()
			elif sheetName=='busSun':
				self.busSun=(pd.read_excel(self.excelFile,sheetName).loc[[0,1,2,8,24,25,26,27,28,29,36,37,38],:]).to_numpy()
			elif sheetName=='busWind':
				self.busWind=(pd.read_excel(self.excelFile,sheetName).loc[[0,1,2,8,24,25,26,27,28,29,36,37,38],:]).to_numpy()
			elif sheetName=='busData':
				self.busData=(pd.read_excel(self.excelFile,sheetName)).to_numpy()
			elif sheetName=='branchData':
				self.branchData=(pd.read_excel(self.excelFile,sheetName).loc[[0,1,2,3,14,29,30,31,32,33,42,43,44],:]).to_numpy()
	...
	## Set Matrices ##
		...
		if 'consDCPF' in self.consList:
			...
			fromTo=np.zeros((46,39))
			for branch in range(Branch):
				#print(fromList[branch])
				fromTo[branch,fromList[branch]]=1
				fromTo[branch,toList[branch]]=-1
			#np.set_printoptions(threshold=np.inf)
			# print(fromList)
			# print(toList)
			# catatan: fromTo delete hanya aktif saat islanding
			fromTo = np.delete(fromTo,np.where(~fromTo.any(axis=0))[0],axis=1) # remove column yang hanya punya nilai 0
			fromTo = fromTo[~np.all(fromTo == 0, axis=1)] # remove row yang hanya punya nilai 0
			# exit()
			...
			# admitanceMatrix=np.zeros((46,39))
			admitanceMatrix=np.zeros((39,39))
			...
			admitanceMatrix = np.delete(admitanceMatrix,np.where(~admitanceMatrix.any(axis=0))[0],axis=1) # remove column yang hanya punya nilai 0
			admitanceMatrix = admitanceMatrix[~np.all(admitanceMatrix == 0, axis=1)] # remove row yang hanya punya nilai 0
